...
// Use the 'upload' middleware for the specific route
router.put('/submit-assignment-screenshot', upload.single('screenshot'), async (req, res) => {
  try {
    // Extract the screenshot, lectureName, and userId from the request body
    ...
    console.log('Received screenshot submission for assignment:', assignmentName);
    console.log('Screenshot:', screenshot); // This will show information abou
    ...
    const mailOptions = {
      from: 'smarttechhubinc@gmail.com',
      to: 'smarttechhubinc@gmail.com', // Admin's email address
      subject: 'Assignment Screenshot Submission',
      text: `Assignment Name: ${assignmentName}`,
      attachments: [
        {
          filename: 'screenshot.jpg', // Change the filename if needed
          content: screenshotBase64, // Use the screenshotBase64 string
    ...
    transporter.sendMail(mailOptions, async (error, info) => {
      if (error) {
        ...
        // Here, after successfully sending the email, update the assignment completion status
        const updateResult = await updateAssignmentCompletion(assignmentName, userId);
        ...
        console.log('Email sent:', info.response);
        res.status(200).json({ message: 'Assignment screenshot submitted successfully' });
      }
    });
  } catch (error) {
    console.error('Error submitting assignment screenshot:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
